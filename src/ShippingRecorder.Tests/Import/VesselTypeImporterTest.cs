...
{
    [TestClass]
    public class VesselTypeImporterTest
    {
        private const string Name = "Passenger (Cruise) Ship";
        ...
        {
            var importer = new VesselTypeImporter(_factory, ExportableVesselType.CsvRecordPattern);
            ...
            var vesselTypes = await _factory.VesselTypes.ListAsync(x => true, 1, int.MaxValue).ToListAsync();
            Assert.HasCount(1, vesselTypes);
            Assert.AreEqual(Name, vesselTypes.First().Name);
        }
        ...
        {
            const string Name = "Passenger (Cruise) Ship";
            await _factory.VesselTypes.AddAsync(Name);
            _filePath = Path.ChangeExtension(Path.GetTempFileName(), "csv");
