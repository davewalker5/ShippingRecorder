...
namespace ShippingRecorder.BusinessLogic.Database
{
    ...
    internal abstract class ReportManagerBase : DatabaseManagerBase
    {
        ...
        protected async Task<IEnumerable<T>> GenerateReportAsync<T>(string query, int pageNumber, int pageSize) where T : class
        {
            ...
            var all = await _factory.GetContext<ShippingRecorderDbContext>().Set<T>().FromSqlRaw(query).ToListAsync();
        ...
        protected static string ReadSqlResource(string file, Dictionary<string, string> placeHolderValues)
        {
            ...
            var assembly = Assembly.GetExecutingAssembly();
            var sqlResourceName = $"ShippingRecorder.BusinessLogic.Sql.{file}";
