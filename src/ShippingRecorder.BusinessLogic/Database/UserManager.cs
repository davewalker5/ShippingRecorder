...
{
    internal class UserManager : DatabaseManagerBase, IUserManager
    {
        private readonly Lazy<PasswordHasher<string>> _hasher;

        internal UserManager(IShippingRecorderFactory factory) : base(factory)
        {
        ...
        public IAsyncEnumerable<User> GetUsersAsync() =>
            Context.Users.AsAsyncEnumerable();
        ...
        public async Task<User> AddUserAsync(string userName, string password)
        {
            ...
            _factory.Logger.LogMessage(Severity.Debug, $"Added user {user}");
            return user;
        }
