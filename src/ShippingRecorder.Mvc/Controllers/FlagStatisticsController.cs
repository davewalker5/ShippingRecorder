...
{
    [Authorize]
    public class FlagStatisticsController : ShippingRecorderControllerBase
    {
        private readonly IReportsClient _reportsClient;
        private readonly IShippingRecorderApplicationSettings _settings;

        public FlagStatisticsController(
            IReportsClient iReportsClient,
            IShippingRecorderApplicationSettings settings,
            IPartialViewToStringRenderer renderer,
            ILogger<FlagStatisticsController> logger) : base (renderer, logger)
        {
        ...
        {
            FlagStatisticsViewModel model = new FlagStatisticsViewModel
            {
                PageNumber = 1
            };
            return View(model);
        ...
        public async Task<IActionResult> Index(FlagStatisticsViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                List<FlagStatistics> records = await _reportsClient.FlagStatisticsAsync(model.From, model.To, page, _settings.SearchPageSize);
