...
{
    [Authorize]
    public class LocationStatisticsController : ShippingRecorderControllerBase
    {
        ...
        public async Task<IActionResult> Index(LocationStatisticsViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                List<LocationStatistics> records = await _reportsClient.LocationStatisticsAsync(model.From, model.To, page, _settings.SearchPageSize);
