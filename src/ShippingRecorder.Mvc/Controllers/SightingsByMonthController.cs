...
{
    [Authorize]
    public class SightingsByMonthController : ShippingRecorderControllerBase
    {
        private readonly IReportsClient _reportsClient;
        private readonly IShippingRecorderApplicationSettings _settings;

        public SightingsByMonthController(
            IReportsClient iReportsClient,
            IShippingRecorderApplicationSettings settings,
            IPartialViewToStringRenderer renderer,
            ILogger<SightingsByMonthController> logger) : base (renderer, logger)
        {
        ...
        {
            SightingsByMonthViewModel model = new SightingsByMonthViewModel
            {
                PageNumber = 1
            };
            return View(model);
        ...
        public async Task<IActionResult> Index(SightingsByMonthViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                List<SightingsByMonth> records = await _reportsClient.SightingsByMonthAsync(start, end, page, _settings.SearchPageSize);
